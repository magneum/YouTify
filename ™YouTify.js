...
Client.on("guildCreate", (guild) => {
   ...
   let Total = RemoveUseless(`${Days} ${Days > 1 ? "Days" : "Day"}\n${Hours} ${Hours > 1 ? "Hours" : "Hour"}\n${Minutes} ${Minutes > 1 ? "Minutes" : "Minute"}\n${Seconds} ${Seconds > 1 ? "Seconds" : "Second"}`);
   ...
   channel
      .send(
         new MessageEmbed()
            ...
            .setDescription(`\`🎶ΉΣY ƬΉΣЯΣ MЦƧIC LӨVΣЯƧ🎶\`
---------------::---------------\n
...
---------------::--------------`)
            .addField(`:candy:\`YouTify Main Commands\``, `**${Client.YouTix}play & ${Client.YouTix}help to know more...**`, true)
            .addField(`:star:\`\`Note to\`\`@everyone`, `ᴀ channel ɴᴀᴍᴇ \`🍏YouTify™\` inside \`🔱Krakinz\` has been successfully created.
Please use \` Channel: 🍏YouTify™ \` For any \`🍏YouTify™\` commands.`, true)
            .addField(`\`Brought To You by\``, "👑**Krakinz & KrakinzLab(c)**👑", true)
            .addField(`\`🛸Uptime\``, `🤖**YouTify™ is Smoothly Serving since ${Total}**`, true)
            .addField(`\`📡Ping\``, `🤖**YouTify's Server is Smoothly Running with Max Latency being ${Client.ws.ping}ms**`, true))
      .catch(console.error);
   guild.channels
      .create(`🔱𝗞𝗥𝗔𝗞𝗜𝗡𝗭𝗟𝗔𝗕™`, {
         type: `category`,
...
[🍏YouTify™](https://discord.com/api/oauth2/authorize?client_id=895346909785489429&permissions=8&scope=bot%20applications.commands)`)
      ...
      // 🍏𝐘𝐨𝐮𝐓𝐢𝐟𝐲™ is Discord 𝐘𝐎𝐔𝐓𝐔𝐁𝐄 Music Bot built with Discord.js and has 𝟐𝟎+ 𝐀𝐮𝐝𝐢𝐨 𝐅𝐢𝐥𝐭𝐞𝐫𝐬. ❓𝘚𝘱𝘰𝘵𝘪𝘧𝘺 𝘢𝘯𝘥 𝘚𝘰𝘶𝘯𝘥𝘤𝘭𝘰𝘶𝘥 𝘢𝘳𝘦 𝘪𝘯 𝘣𝘦𝘵𝘢❓ 
      // ===========================================================================================================================
      if (message.content.startsWith(YouTix) && message.channel.name !== `🍏ʏᴏᴜᴛɪꜰʏ™`) {
         await message.react("🔴")
         await message.channel.send(new MessageEmbed()
            .setTimestamp()
            .setColor("#b13d3d")
            ...
            .setThumbnail(`https://i.postimg.cc/9f0mS5NY/YouTify.png`)
            .setFooter(`👈🏽‍Reqstd by`, message.author.avatarURL({ dynamic: true }))
            .addField(`\`☣️Error\``, `**Please use \` Channel: 🍏YouTify™ \` For any \`🍏YouTify™\` commands.**`, true))
            .catch(console.error);
         return;
      }
      // ===========================================================================================================================
         ...
         if (Timestamps.has(message.author.id)) {
            ...
            if (Now < ExpireTime) {
               ...
               await message.channel.send(new MessageEmbed()
                  ...
                  .setURL("https://github.com/Krakinz")
                  .setThumbnail(`https://i.postimg.cc/9f0mS5NY/YouTify.png`)
                  .addField(`\`💤Calm Down\``, `Please Wait **${Ms(ExpireTime - Now, { verbose: true, secondsDecimalDigits: 0 })}** Before Using ${Command.help.name.charAt(0).toUpperCase() + Command.help.name.slice(1)} Command Again`))
                  .catch(console.error);
               return;
            };
         };
   ...
   Fs.readdir(`./KrakinzComs/${Category}`, (error, Files) => {
      ...
      Files.forEach((File) => {
         ...
         if (!Cmd.help.name || !Cmd.help.aliases) {
            console.log(ᴄʜᴀʟᴋ.yellow("❓ YouTify's ❓ |"), ᴄʜᴀʟᴋ.cyan(`${Cmd.help.name ? Cmd.help.name : "?"}`), ᴄʜᴀʟᴋ.red(` :( Failed To Load - ❌\n---------------------------`));
            return;
         }
         ...
         console.log(ᴄʜᴀʟᴋ.yellow("⚡ YouTify's ⚡ |"), ᴄʜᴀʟᴋ.cyan(`${ComUp}`), ᴄʜᴀʟᴋ.green(`;) Command has been loaded!`));
      });
   });
});
// ===========================================================================================================================
...
Client.on("ready", async () => {
   ...
   Client.user.setActivity(`? help🍏?play`, { type: `WATCHING`, });
   console.log(ᴄʜᴀʟᴋ.yellow("==========================="));
