...
module.exports.run = async (client, message, args, Discord) => {
    ...
    if (!args[0]) {
        ...
        await message.channel.send(new MessageEmbed()
            ...
            .addField(`\`💡Sent\``, `**Please Check Your \`Direct Message\` For An Entire Command Helper Menu Of YouTify™**`, true))
            .catch(console.error);
        // ===========================================================================================================================
        // 🍏𝐘𝐨𝐮𝐓𝐢𝐟𝐲™ is Discord 𝐘𝐎𝐔𝐓𝐔𝐁𝐄 Music Bot built with Discord.js and has 𝟐𝟎+ 𝐀𝐮𝐝𝐢𝐨 𝐅𝐢𝐥𝐭𝐞𝐫𝐬. ❓𝘚𝘱𝘰𝘵𝘪𝘧𝘺 𝘢𝘯𝘥 𝘚𝘰𝘶𝘯𝘥𝘤𝘭𝘰𝘶𝘥 𝘢𝘳𝘦 𝘪𝘯 𝘣𝘦𝘵𝘢❓ 
        // ===========================================================================================================================
        ...
        message.author.send(new MessageEmbed()
            ...
            .setAuthor(`🍏YouTify™`, `https://i.postimg.cc/gcX8075z/guitar-sing.gif`)
            .setFooter(`👈🏽‍Reqstd by ${message.author.username}`, message.author.avatarURL({ dynamic: true }))
            .setDescription(`**💡This is the Entire Command Helper Menu for YouTify™**💡\n:exclamation:*For more help type ${client.YouTix}help <command name>*\n================🍏================\n\n`)
            .addField(`**\`PREFIX\`**`, `**${client.YouTix}**`, true)
            .addField(`**\`🎹YOUTIFY PLAYER\`**`, `**play,clean,\nclearqueue,join,\nleave,loop,\nlyrics,nowplaying,\nqueue,search,\nshuffle,skip,volume**`, true)
            .addField(`**\`⚙️SYSTEM\`**`, `**help,invite,\nping,uptime**`, true)
            .addField(`**\`🎶FILTERS\`**`, `**filters,bassboost,nightcore**`, true)
    ...
    if (args[1] && args[1] == client.token) {
        ...
        message.channel.send(new MessageEmbed()
            ...
            .setFooter(`👈🏽‍Reqstd by ${message.author.username}`, message.author.avatarURL({ dynamic: true }))
            .addField("Usage", Cmd.usage ? client.YouTix + Cmd.usage : "None", true)
            .addField("Examples", Cmd.examples ? Cmd.examples.join("\n").length === 0 ? "None" : Cmd.examples.map(YouC => client.YouTix + YouC).join("\n") : "None")
            .setTimestamp())
            .catch(console.error);
        return;
    };
    ...
    await message.channel.send(new MessageEmbed()
        ...
        .addField(`**\`🔎Example\`**`, Cmd.examples ? Cmd.examples.join("\n").length === 0 ? "None" : Cmd.examples.join("\n") : "None"))
        .catch(console.error);
    return;
};
// ===========================================================================================================================
