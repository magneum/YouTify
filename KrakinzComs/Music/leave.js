...
module.exports.run = async (client, message, args) => {
    ...
    if (!MemBot) {
        ...
        await message.channel.send(new MessageEmbed()
            ...
            .setThumbnail(`https://i.postimg.cc/9f0mS5NY/YouTify.png`)
            .setFooter(`👈🏽‍Reqstd by ${message.author.username}`, message.author.avatarURL({ dynamic: true }))
            .addField(`\`☣️Error\``, `**YouTify™ is not connected to any Voice Channel! Use ${client.YouTix}join or ${client.YouTix}play command first**`, true))
            .catch(console.error);
        return;
    }
    // ===========================================================================================================================
    ...
    // 🍏𝐘𝐨𝐮𝐓𝐢𝐟𝐲™ is Discord 𝐘𝐎𝐔𝐓𝐔𝐁𝐄 Music Bot built with Discord.js and has 𝟐𝟎+ 𝐀𝐮𝐝𝐢𝐨 𝐅𝐢𝐥𝐭𝐞𝐫𝐬. ❓𝘚𝘱𝘰𝘵𝘪𝘧𝘺 𝘢𝘯𝘥 𝘚𝘰𝘶𝘯𝘥𝘤𝘭𝘰𝘶𝘥 𝘢𝘳𝘦 𝘪𝘯 𝘣𝘦𝘵𝘢❓ 
    // ===========================================================================================================================
    if (!message.member.hasPermission("MANAGE_CHANNELS") && MemBot.members.length > 2) {
        await message.react("🔴")
        await message.channel.send(new MessageEmbed()
            .setTimestamp()
            .setColor("#b13d3d")
